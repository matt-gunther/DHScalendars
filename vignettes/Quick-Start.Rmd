...
In most cases, you should be able to process a new sample in just a few minutes if you follow these steps:
  ...
  4. Use the function `make_lr()` to process your sample. 
...
The availability of calendars for each sample is recorded in this CSV file. **Enter the availability of calendars for your sample in the next available row**. 
...
Once you've identified the reproductive events calendar `vcal_reprod` , you'll need to check to see if it contains any sample-specific codes. **Check the sample documentation on the DHS website for hints.**
...
Once you've identified the discontinuation calendar `vcal_discont` , you'll need to check to see if it contains any sample-specific codes. **Check the sample documentation on the DHS website for hints.**
...
samples <- read_csv("resources/vcal_availability.csv") %>% 
  pull(SAMPLE)
