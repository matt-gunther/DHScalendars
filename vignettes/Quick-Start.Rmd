...
In most cases, you should be able to process a new sample in just a few minutes if you follow these steps:
  ...
  1. Identify the available calendars and mark them in "resources/vcal_availability.csv"
  2. Identify any sample-specific codes for the **Reproductive Events** calendar, which we'll call `vcal_reprod` (this is usually stored in `vcal_1` in the IR file). Enter sample-specific recoding in "resources/vcal_repord_recodes.csv".
  3. Identify any sample-specific codes for the **Discontinuation** calendar, which we'll call `vcal_discont` (if available, this is usually stored in `vcal_2` in the IR file). Enter sample-specific recoding in "resources/vcal_discont_recodes.csv". 
  4. Use the function `make_lr()` to create an unlinked LR file for your sample.
  5. Use the IPUMS python function 
...
Locate the file at "./ipums/dhs/general/calendar/sample_tracking/vcal_availability.csv" (substitute the location of the ipums folder for "." depending on your operating system). 

The availability of calendars for each sample is recorded in this CSV file. **Enter the availability of calendars for your sample in the next available row**. You should not enter the number for any calendar where the only available values are "B" or otherwise missing. Consult the DHS Final Report for your sample for help with this task.
...
**Warning: vcal columns can only store numbers. Please use the number associate with the vcal_ variable for your sample.** In rare cases, you may find calendar data stored in the IR file under an unusual name (see eg2995ir `vcal_term`). In this case, please insert the number 0 in place of the unusual name; you'll need to edit some of the functions in this package to make special accommodations for such a sample (*instructions coming soon*).

# vcal_reprod_recodes.csv

Locate the file at "./ipums/dhs/general/calendar/sample_tracking/vcal_reprod_recodes.csv" (substitute the location of the ipums folder for "." depending on your operating system). 

Once you've identified the reproductive events calendar `vcal_reprod` , you'll need to check to see if it contains any sample-specific codes. Consult the DHS Final Report for your sample for help with this task.
...
Locate the file at "./ipums/dhs/general/calendar/sample_tracking/vcal_discont_recodes.csv" (substitute the location of the ipums folder for "." depending on your operating system). 

Once you've identified the discontinuation calendar `vcal_discont` , you'll need to check to see if it contains any sample-specific codes. Consult the DHS Final Report for your sample for help with this task.
...
Notice that the first few rows of this CSV show recodes that are **common to all samples** (both `sample` and `year` are marked "all"). You do not need to enter any recodes that are common to all samples. Likewise **any numeric input value will be recycled as output automatically** unless you specify otherwise.
...
This function creates an unlinked LR file containing all of the derived calendar variables (including placeholders for those that were not available for your sample).
...
If you're crafty, you can even read a vector of samples from a CSV file. Here's how you might process all of the samples in `vcal_availability.csv` (this would probably take around 2 hours on the RStudio server):

```{r,  eval = F, echo = T}
# I make my to-do list from the CSV file:
samples <- "vcal_availability.csv" %>% 
  file.path("/pkg/ipums/dhs/general/calendar/sample_tracking", . ) %>% 
  read_csv() %>% 
  pull(SAMPLE)
...
{when finished, insert details about the output destination and format here}
