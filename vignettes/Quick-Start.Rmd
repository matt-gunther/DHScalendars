...
In most cases, you should be able to process a new sample in just a few minutes if you follow these steps:
  ...
  2. Identify any sample-specific codes for the **Reproductive Events** calendar, which we'll call `vcal_reprod` (this is usually stored in `vcal_1` in the IR file). Enter sample-specific recoding in `vcal_repord_recodes.csv`.
...
# dhs_calendar_linking
 
Use a Terminal on a general purpose server (e.g. `gp2`) and run the `dhs_calendar_linking` **Python script**. This will create *both* a linked data file *and* and new data dictionary for your new `lr` sample. 

If you provide only one sample, the function will run on the general purpose server (e.g. `gp2`). For example:

`dhs_calendar_linking -s af2015lr`
